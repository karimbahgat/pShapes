...
def scrapecodes(url):
    ...
    if raw:
        ...
        fields = [th.string for th in codes.find_all('th')]
...
for iso2,items in itertools.groupby(sorted(shaperecords, key=lambda x: x[0]["iso_a2"]), key=lambda x: x[0]["iso_a2"]):
    ...
    for nerow,neshp in items:
        ...
        for strow in strows:
            ...
            if strow["Name"].strip() in nenames:
                pass
            elif strow.get("HASC") and strow["HASC"] == nerow["code_hasc"]:
                pass
            elif strow.get("FIPS") and strow["FIPS"] == nerow["fips"]:
                pass
            elif strow.get("ISO") and strow["ISO"] == nerow["iso_3166_2"]:
                pass
            else:
                continue # none of the names or codes matched, so skip to next statoid row

            # match found, so override codes and stop looking
            nerow["code_hasc"] = strow.get("HASC", nerow["code_hasc"])
            nerow["iso_3166_2"] = strow.get("ISO", nerow["iso_3166_2"])
            nerow["fips"] = strow.get("FIPS", nerow["fips"])
            print "updated",repr((nerow["code_hasc"],nerow["iso_3166_2"],nerow["fips"]))
