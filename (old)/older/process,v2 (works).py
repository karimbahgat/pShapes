...
##                subparts.append(change)
        ...
        if len(breakoffs) > 1:
            ...
            if trimmedgeom:
                ...
                if newinfo:
                    prereceiving = PreReceiving(newinfo.fromprov, trimmedgeom)
                else:
                    prereceiving = PreReceiving(toprovid, trimmedgeom)
...
##    # 1.5) Change any fromprovs that use the newer id, if any newinfo (eg prereceiving)
##    newinfo = next((change for change in changes if change.type == "NewInfo"), None)
##    for change in changes:
##        if change.type == "NewInfo":
##            for subpart in subparts:
##                if subpart.fromprov == change.toprov:
##                    subpart.fromprov = change.fromprov


    # 2) Group and union all geom parts by fromprov
    #    NEWINFO SHOULD BE UPDATED SOMEWHERE IN HERE??
    #    COULD BE THAT NEWINFO SHOULD CREATE A NEW FEATURE, BUT ONLY IF ALONE??
    for fromprovid,subparts in itertools.groupby(subparts, key=lambda sb: sb.fromprov):
        subparts = list(subparts)

        # If any breakaways or parttransfers, need to get the
        # ...newer version of fromprov so they can all be unioned
##        anygiving = any((subpart.type in ("PartTransfer","Breakaway") for subpart in subparts))
##        if anygiving:
##            # Lookup the fromprov geometry
##            newinfo = next((subpart for subpart in subparts if subpart.type == "NewInfo"), None)
##            if newinfo:
##                newprovgeom = newinfo.geom
##            else:
##                newprov = find_prov(fromprovid)
##                newprovgeom = shapely.geometry.shape(newprov.geometry)
##            if not newprovgeom:
##                # couldnt find provcode, need better lookup, maybe using multiple ids
##                print("Couldnt find newprov, for now just skipping")
##                continue
##            pregiving = PreGiving(fromprovid, newprovgeom)
##            print "getting pregiving"
##            subparts.append(pregiving)
        ...
        # Set startdate for newer provs
        # MAYBE THIS IS MORE EFFICIENTLY DONE IN THE TOPROV STAGE?
##        for subpart in subparts:
